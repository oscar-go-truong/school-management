...
<body class="bg-gray-100">
    <div class="container mx-auto my-10 p-5 bg-white rounded-lg shadow-md">
        ...
        <p class="text-gray-700 text-base leading-7 mb-6">I am writing to inform you that there has been a change in the
            course of one of our students. {{ $user->fullname }} has recently changed their course from
            {{ $oldCourse->subject->name }}
            {{ $oldCourse->name }} to {{ $newCourse->subject->name }}
            {{ $newCourse->name }}. This change was made on {{ $date }} approve by {{ $userApprove->fullname }}
            with reason {{ $reason }}.</p>
        ...
        <p class="text-gray-700 text-base leading-7 mb-6">Once again, congratulations on your enrollment, and we look
            forward to seeing you in class soon!</p>
